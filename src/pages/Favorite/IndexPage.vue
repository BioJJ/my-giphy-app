<template>
  <div class="flex flex-col items-center justify-center">
    ...
    <q-page class="flex flex-col items-center w-full">
      <div v-if="favorites.length === 0" class="text-2xl mt-8">Sem Favoritos...</div>
      <div
        v-else
        class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 w-full"
      >
        <div v-for="gif in favorites" :key="gif.id" class="flex flex-col items-center">
          <img
          ...
          />
          <q-btn
            @click="() => store.removeFromFavorites(gif.id)"
            label="Remover"
